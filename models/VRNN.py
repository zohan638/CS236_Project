...
class VRNN(nn.Module):
    def __init__(self, kwargs, bias=False):
        ...
        if kwargs['rnn_type'] == 'GRU':
            self.rnn = nn.GRU(self.hid_dim + self.hid_dim, self.hid_dim, self.n_layers, self.bias, bidirectional=False)
        elif kwargs['rnn_type'] == 'LSTM':
            self.rnn = nn.LSTM(self.hid_dim + self.hid_dim, self.hid_dim, self.n_layers, self.bias, bidirectional=False)
    ...
    def forward_LSTM(self, inputs, lengths):
        ...
        recon_loss = 0.0
        # h = torch.zeros(self.n_layers, x.shape[1], self.hid_dim, device=self.device)
        h = torch.zeros(self.n_layers, batch_size, self.hid_dim, device=self.device)
        c = torch.zeros(self.n_layers, batch_size, self.hid_dim, device=self.device)
    ...
    def sample_LSTM(self, seq_len):
        ...
        h = torch.zeros(self.n_layers, 1, self.hid_dim, device=self.device)
        c = torch.zeros(self.n_layers, 1, self.hid_dim, device=self.device)
